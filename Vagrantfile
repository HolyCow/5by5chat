...
Vagrant.configure("2") do |config|
  ...
  config.vm.box_url = "http://files.vagrantup.com/precise32.box"
   ...
   config.vm.provision :shell,
    :inline => "sudo apt-get -y install curl libpq-dev postgresql"
   ...
   config.vm.provision :shell,
    :inline => "sudo -u postgres psql -c \"CREATE USER vagrant WITH PASSWORD 'vagrant' CREATEDB\""

   config.vm.provision :shell,
    :inline => "createdb 5by5chatbot"
